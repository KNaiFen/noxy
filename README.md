...
**测试前提：**同一个种子的主世界，服务端已预生成地形和 LOD；一名空缓存客户端固定在原点，接收半径 **256 区块**内的 **205,548 列远景**（另有 313 列由原版近景提供）。两次测试都设置 **30 Mbps** 服务端上传上限。只比较压缩后的 **LOD 数据载荷**，不包含原版区块、协议开销等其他流量。旧全 L0 基准与当前 0.2.16 测试分别在不同日期、不同版本的服务端缓存上完成；下表是同种子、同范围的参考对照，不是严格的同轮 A/B。

| 半径 256 区块 | 旧基准：全部按 L0 发送 | 当前：默认分档＋16 列合批 |
| --- | ---: | ---: |
| LOD 数据载荷 | 1,195.71 MB | 117.93 MB |
| 相比全部 L0 少传 | — | **1,077.78 MB（90.14%）** |

当前测试使用预生成并导入 LOD 的 **512×512 区块**服务端存档。空缓存客户端从首片到末片约 **175 秒**，平均约 **1,172 列/秒、5.38 Mbps**；从开启接收到队列完全稳定约 **488 秒**，之后覆盖审计确认半径内 **205,861 列**全部达标。30 Mbps 是上限，实测平均没有跑满。每名新玩家少下载约 1.08 GB 的 LOD，服务端也少占用相应出口流量，能为其他玩家和正常游戏留出带宽。服务端 LOD 数据库占 **2.06 GB**，客户端接收后 `.voxy` 缓存占 **153.34 MB**；两者覆盖范围和存储格式不同，不能用大小差当成磁盘节省率。结果来自本机单客户端、固定视角和已有服务端缓存；其他地图、未生成地形和多人同时接收会改变耗时。
