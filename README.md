...
当前测试使用预生成并导入 LOD 的 **512×512 区块**服务端存档。空缓存客户端从首片到末片约 **175 秒**，平均约 **1,172 列/秒、5.38 Mbps**；从开启接收到队列完全稳定约 **488 秒**，之后覆盖审计确认半径内 **205,861 列**全部达标。30 Mbps 是上限，实测平均没有跑满。每名新玩家少下载约 1.08 GB 的 LOD，服务端也少占用相应出口流量，能为其他玩家和正常游戏留出带宽。服务端 LOD 数据库占 **2.06 GB**，客户端接收后 `.voxy` 缓存占 **153.34 MB**。结果来自本机单客户端、固定视角和已有服务端缓存；其他地图、未生成地形和多人同时接收会改变耗时。
